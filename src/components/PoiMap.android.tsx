...
const webMapHtml = (poi: PointOfInterest, reduceMotion: boolean) => {
  ...
  return `<!doctype html><html><head><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" />
    ...
    window.centerOnPoi=function(){map.setView([point.lat,point.lng],16,{animate:${!reduceMotion}});};</script></body></html>`;
};
    ...
    <>
      <View style={styles.frame}>
        {Platform.OS === 'android' ? <WebView
          key={poi.id}
          originWhitelist={['*']}
          source={{ html: webMapHtml(poi, reduceMotion), baseUrl: 'https://unpkg.com' }}
          javaScriptEnabled
          domStorageEnabled
          style={styles.map}
        /> : <MapView
