...
### ผลวัดก่อนรวมการแก้ Expo Go (Profiler export ชุดแรก)
...
ตัวเลขเป็นหลักฐานจากเวอร์ชัน Week 11 และ Week 12 ก่อนรวมการแก้ Expo Go บน Android ครั้งนี้ จึงไม่ใช่ผลวัดของ commit ล่าสุด หากต้องการเปรียบเทียบเวอร์ชันล่าสุด ให้เก็บ Profiler ใหม่บนเครื่องและขั้นตอนเดียวกัน

### Profiler export ที่ผู้ทดสอบส่งหลังรวมการแก้ Android

อ่านจากไฟล์ React DevTools `week11 test(1).json` และ `week12 test(1).json` ที่ผู้ทดสอบ export บน iPhone; ภาพหน้าจอแสดง commit รายตัว 0.5 ms และ 0.6 ms ตามลำดับ ตัวเลขนั้นเป็นเพียง commit แรกของแต่ละไฟล์ (`PressableText`) จึงใช้ตัดสินความเร็วทั้งแอปไม่ได้

| สิ่งที่อ่านได้จากไฟล์ | Week 11 | Week 12 |
|---|---:|---:|
| จำนวน commits ในช่วงที่บันทึก | 25 | 107 |
| commit เปิดหน้าผ่าน `BaseNavigationContainer` (ลำดับ 3 ในไฟล์) | 33.874 ms | 23.256 ms |
| commit กลับหน้า Event list ผ่าน navigation (ลำดับ 20 และ 35) | 9.826 ms | 6.802 ms |
| commit อัปเดต Event list หลังกลับหน้า (ลำดับ 21 และ 36) | 5.822 ms | 4.749 ms |
| commits ที่ระบุ `VirtualizedList` เป็น updater | 1 | 81 |

ลำดับในตารางนับจาก 1 แต่ index ใน JSON เริ่มที่ 0 ทั้งสองไฟล์บันทึกประมาณ 10 วินาทีและมีการนำทางหลายหน้า; Week 12 มี 84 commits ที่รายงาน render duration เป็น `0 ms` จากการจัดการ list การนับจำนวน commits หรือรวมเวลา render ทั้งช่วงจึงไม่ใช่การเทียบงานปริมาณเท่ากัน ผลที่เห็นคือ **commit การนำทางที่เลือกมาของ Week 12 ใช้เวลาน้อยกว่าในรอบนี้** ส่วน commit อัปเดต Event list ครั้งแรกหลังเปิดหน้ากลับใช้ 8.301 ms เทียบกับ 5.857 ms ใน Week 11 จึงยังสรุปไม่ได้ว่าทุก interaction เร็วขึ้น

ข้อมูลชุดนี้ไม่มีการระบุจังหวะพิมพ์และจำนวนกิจกรรมที่ตรงกันอย่างชัดเจน จึงไม่ใช้ยืนยันค่าเฉลี่ยตอนพิมพ์, FPS ของการเลื่อน, หรือผลรวมของแอป ถ้าต้องการผลเทียบที่ทำซ้ำได้ ให้บันทึก interaction เดียวกันด้วยจำนวนกิจกรรมเท่ากันหลายรอบบนเครื่องและโหมด build เดียวกัน

### ทดสอบพิมพ์รอบใหม่หลังรวมการแก้ Android

ผู้ทดสอบ export ไฟล์ `week11 test(2).json` และ `week12 test(2).json` จาก React DevTools บน iPhone ทั้งคู่มี commit ที่ระบุ `InternalTextInput` เป็น updater **อย่างละ 5 ครั้ง** จึงเทียบเวลาที่ React ใช้ render commit ช่วงพิมพ์ในแต่ละไฟล์ได้ โดย index ในวงเล็บเริ่มที่ 0:

- Week 11 (15–19): `12.470, 16.076, 14.179, 15.634, 14.643 ms`
- Week 12 (19, 21, 23, 25, 27): `8.217, 11.347, 13.447, 13.594, 13.668 ms`

| เวลา render ของ 5 input commits | Week 11 | Week 12 | ผลในรอบนี้ |
|---|---:|---:|---:|
| ค่าเฉลี่ยต่อ commit | 14.600 ms | 12.055 ms | Week 12 น้อยกว่า 17.4% |
| มัธยฐาน | 14.643 ms | 13.447 ms | Week 12 น้อยกว่า 8.2% |
| ค่าสูงสุด | 16.076 ms | 13.668 ms | Week 12 น้อยกว่า 15.0% |
| ผลรวม 5 commits | 73.002 ms | 60.273 ms | Week 12 น้อยกว่า 17.4% |

ตัวเลขนี้เป็น **render duration ของ React** ไม่ใช่เวลาตอบสนองตั้งแต่แตะแป้นพิมพ์จนเห็นข้อความหรือ FPS ของแอป ไฟล์ยืนยันได้ว่าเกิด input commits อย่างละ 5 ครั้ง แต่ไม่เก็บตัวอักษรที่พิมพ์หรือจำนวนกิจกรรมที่แสดง จึงควรตีความเป็นผลของรอบทดสอบนี้และเก็บซ้ำหากต้องการข้อสรุปที่มั่นคง

ทั้งไฟล์มี 22 commits ใน Week 11 และ 86 commits ใน Week 12; Week 12 มี 64 commits ที่ระบุ `VirtualizedList` เป็น updater โดยหลายรายการแสดง render duration เป็น `0 ms` จำนวน commits และเวลารวมทั้งไฟล์จึงใช้เทียบความเร็วการเลื่อนหรือความเร็วทั้งแอปตรง ๆ ไม่ได้
